...
export function TradesOverlays() {
  const { user } = useAuth();
  const { requests, panelOpen, closePanel, accept, decline, openChat } = useTrades();
  if (!user) return null;
  return (
    <>
      <TradeRequestsPanel open={panelOpen} onClose={closePanel} requests={requests} myUid={user.uid} onAccept={accept} onDecline={decline} onChat={openChat} />
      <ChatDrawer />
    </>
  );
}
